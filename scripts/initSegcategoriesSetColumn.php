...
$sql = "SELECT * FROM `segcategories` WHERE set_id = 1";
...
$sql2 = "SELECT * FROM `segsets` WHERE id != 1";
...
if (mysqli_num_rows($sets) > 0) {
	while($row = mysqli_fetch_assoc($sets)) {
		...
		foreach ($catList as &$cat) {
			...
			$currentSetId = $row["id"];
			$sql3 = "SELECT * FROM `segcategories` WHERE `Category` = '$currentCatCat' AND `set_id` = '$currentSetId'";
			$res3 = mysqli_query($link, $sql3);
			if (mysqli_num_rows($res3) == 0) {
				$sql4 = "INSERT INTO `segcategories` (`id`, `Category`,`Color`, `set_id`) VALUES (NULL, '$currentCatCat', '$currentCatColor', '$currentSetId')";
