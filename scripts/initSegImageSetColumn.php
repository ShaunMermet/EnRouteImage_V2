...
$sql = "SELECT * FROM `segImages` WHERE 1";
...
if (mysqli_num_rows($imgs) > 0) {
	while($row = mysqli_fetch_assoc($imgs)) {
		...
		if(is_null($catID) || $catID == 0){
	        ...
	        $sql2 = "SELECT Category FROM `segcategories` WHERE id = $catID";
		...
		$sql3 = "SELECT * FROM `segsets` WHERE `name` = '$catName' AND `group_id` = '$grpID'";
		$res3 = mysqli_query($link, $sql3);
		if (mysqli_num_rows($res3) == 0) {
			error_log("create ".$catName." - ".$grpID);
			$sql4 = "INSERT INTO `segsets` (`id`, `name`, `group_id`) VALUES (NULL, '$catName', '$grpID')";
		...
		error_log($setId);
		$sql6 = "UPDATE `segImages` 
	    SET `set_id`='$setId'
	    WHERE `segImages`.`id` = '$rowID'";
